class ApiEndpoints {
  static const signup = '/auth/local/signup';
  static const signin = '/auth/local/signin';
  static const retriveUser = '/auth/user';
  static const editUser = '/auth/user/edit';
  static const changePassword = '/auth/user/password';
  static const refreshToken = '/auth/refresh';
  static const signout = '/auth/signout';
  static const getCategories = '/category/all';
  static const createCategory = '/category/create';
  static const editCategory = '/category';
  static const deleteCategory = '/category';
  static const getTodos = '/todo/all';
  static const createTodo = '/todo/create';
  static const editTodo = '/todo';
  static const deleteTodo = '/todo';
  static const changeStatus = '/todo/status';
}
